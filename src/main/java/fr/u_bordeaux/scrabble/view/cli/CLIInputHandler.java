        ...
        try {
            ...
            if (posInput[0].matches("[a-oA-O]")) {
                ...
                for (int i = 0; i < rows.length; i++) {
                    if (rows[i].equals(row)) { rowIndex = i; break; }
                }
                if (rowIndex == -1) throw new IllegalArgumentException("Ligne invalide: " + posInput[0]);
                y = rowIndex;
            ...
            for (char letter : lettersInput.toCharArray()) {
                ...
                for (Tile tile : rack) {
                    if (tile.getCharacter() == letter) {
                        ...
                        for (Tile t : tiles) {
                            if (t == tile) { alreadyUsed = true; break; }
                        }
                        if (!alreadyUsed) { tiles.add(tile); found = true; break; }
                    }
                }
                if (!found) {
