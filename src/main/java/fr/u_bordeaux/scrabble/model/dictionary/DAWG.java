...
public class DAWG {
    ...
    // Registry of unique minimized nodes (the heart of the DAWG's memory efficiency)
     ...
     * WARNING: Words must be added in alphabetical order for correct minimization.
     */
    public void add(String word) {
        ...
        for (int i = 0; i < Math.min(word.length(), lastWord.length()); i++) {
            if (word.charAt(i) != lastWord.charAt(i)) break;
            commonPrefix++;
        }
    ...
    /**
     * Searches for all words constructible with a given rack.
     * If a hook is specified, it is added to the rack and the word MUST contain it.
     */
    public Set<String> findWordsWithRackAndHook(Character[] rack, char hook) {
        ...
        if (hook != ' ') availableLetters.add(hook);
        ...
        if (hook != ' ') {
            for (String word : allFound) {
                if (word.indexOf(hook) != -1) results.add(word);
            }
            return results;
        }
        return allFound;
    ...
    private void backtrack(DAWGNode node, String currentWord, List<Character> rack, Set<String> results) {
        if (node.getFinite()) {
