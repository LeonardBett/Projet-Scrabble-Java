    ...
    broadCastThread = new Thread(() -> {
      try (DatagramSocket broadcastSocket =
        ...
        DatagramPacket packet = new DatagramPacket(byteMessage, byteMessage.length,
            destinationAddress, DEFAULT_UDP_PORT);
  ...
  public void startListening() {
    ...
    new Thread(() -> {
      try {
        ...
        listenSocket.bind(
            new java.net.InetSocketAddress(InetAddress.getByName("0.0.0.0"), DEFAULT_UDP_PORT));
        ...
        if (isListening) {
          System.err
              .println("Listening : Socket Exception caught with message : " + e.getMessage());
        }
      } catch (Exception e) {
