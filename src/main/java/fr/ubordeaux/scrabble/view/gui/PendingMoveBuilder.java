...
public class PendingMoveBuilder {
  ...
  /**
   * When only one tile is placed, infers the word direction by checking which axis has adjacent
   * tiles already on the board. If only vertical neighbours exist, returns VERTICAL. Otherwise
   * defaults to HORIZONTAL (covers the no-neighbour / horizontal-only cases).
   *
   * @param p     the position of the single placed tile
   * @param board the current board state (may be null, in which case HORIZONTAL is returned)
   * @return the inferred Direction
   */
